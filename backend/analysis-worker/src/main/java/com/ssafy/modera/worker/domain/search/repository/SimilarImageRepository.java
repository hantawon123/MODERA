...
public class SimilarImageRepository {
     ...
     *
     * <p>0.78 → 0.72 (2026-08-09): 운영 DB 전 사용자 이미지쌍 2,218개의 코사인
     * 분포를 실측하고 경계 구간 쌍들의 요약문을 눈으로 대조해 정했다.
     *   무관 baseline: p50=0.529, p90=0.626 (같은 사용자 임의 쌍 대부분이 이 아래)
     *   0.70~0.80 구간 표본: 전부 실제 연관 (걸음수↔걸음수 0.749, SSAFY 지원↔인재상 0.744)
     *   0.65~0.70 구간 표본: 절반이 무관 (팝업스토어 기획↔취업 가이드 0.699)
     *   0.60~0.65 구간 표본: 대부분 무관 (SSAFY 합격↔대학 축제 0.649)
     * 기존 0.65는 혼재 구간을 통과시켜 무관 이미지가 연관으로 떴고, 처음 잡았던
     * 0.78은 0.70~0.78의 확실한 연관(걸음수 쌍 0.749 등)까지 잘랐다. 경계는
     * 0.70이고, 혼재 구간 바로 위인 0.72로 둔다. 조정할 일이 생기면 감으로
     * 만지지 말고 같은 방법(쌍 분포 + 경계 구간 요약문 대조)으로 재실측할 것.
     */
     ...
     *
     * <p>0.65 → 0.70 (2026-08-09): 단일 검색과 같은 실측(SINGLE_MIN_SCORE 주석)에서
     * 0.65~0.70 구간은 절반이 무관으로 나왔다 — 진짜 연관이 모자란 사용자는 top-10
     * 꼬리가 이 혼재 구간으로 채워져 "관련 있어요"에 무관 이미지가 떴다. 실측 경계인
     * 0.70으로 올려 혼재 구간을 제거한다("추천은 확실한 것만" — 팀 결정).
     *
     * <p>단일 검색(0.72)보다 소폭 낮게 두는 이유: 고르는 화면이라 후보가 마르는
     * 비용이 정크 몇 개보다 크고, 상위 N 컷(LIMIT)이 노출을 이미 제한한다.
     */
    private static final double MULTI_MIN_SCORE = 0.70;
