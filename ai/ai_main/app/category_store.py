...
문서는 두 종류다.
  ...
  사용자 문서  id="{user_id}:{정규화이름}"  count>=1
    ...
    쌓일수록 정확해진다. centroid 는 사용자별로 격리된다 — 그 사람 이미지
    내용의 평균이라 개인 데이터다. 다만 **이름은 전역으로 공유한다**
    (2026-08-06 확정, `_add_shared_names`): 아이콘·Spring taxonomy 가 이미
    이름 단위 전역이라 판정 후보만 사용자별이면 같은 개념이 사람마다 다른
    이름으로 갈라진다.

  전역 시드   id="0:{정규화이름}"           count=0
    기본 카테고리 **이름 문자열**의 임베딩. 이미지가 0장인 카테고리는 평균낼
    것이 없어 centroid 를 만들 수 없으므로, 콜드 스타트에서만 대신 쓴다.
    사용자와 무관하게 같은 값이라 사용자별로 복제하지 않는다(문서 13개로 끝).
...
# 한 번에 읽어오는 카테고리 수 상한. 시드 13종에 사용자 카테고리가 더해지는
...
# 이름 임베딩 0.90)이고, 그게 제대로 돌면 서로 다른 이름은 기본 13종에 주제 몇십
