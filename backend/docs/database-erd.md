...
| `image_document` | 이미지 N:M 문서 | `(document_id, image_id)` | 문서에 사용된 이미지 관계. `added_at`으로 문서에 추가된 시점을 기록하고 `del_yn`으로 관계를 soft delete한다. |
...
| `document_image_view` | PK `image_document_id`<br/>인덱스 `(document_id, added_at, image_document_id)` | 특정 문서에 포함된 이미지 목록 | `image_document`, `image_asset`, `thumbnail`, 분석 결과, 태그 | 문서 화면에 필요한 이미지 제목·요약·썸네일·태그를 보관한다. `added_at` 기준 정렬이 가능하며 `del_yn`으로 조회 행을 soft delete한다. |
