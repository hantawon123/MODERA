...
CATEGORY_POLICY: dict[str, Any] = {
    "categories": {
        ...
        # 쇼핑↔음식 경계(2026-08-03 실측): 배달 주문내역·영수증 3장이 "음식=배달"과
        # "쇼핑=주문 화면"에 동시에 걸려 라벨러 4명이 4답(쇼핑/영수증/음식/주문내역).
        # 모델은 일관되게 음식을 골랐으므로 정의 문구를 그 방향으로 못박는다.
        "쇼핑": "실물 상품의 정보·주문·구매 화면 (음식 배달·식당 주문은 '음식')",
        "음식": "맛집·메뉴·레시피·배달, 음식 주문 내역·영수증",
    ...
    # ⚠️ few_shot 에 "접수·예약·모임 → 일정" 류 대비쌍을 추가하지 말 것(8/04 실측 기각):
    # 표적 2건(원서접수·모임 안내)은 고쳤지만 "예약 화면"까지 일정으로 끌어당겨
    # 방탈출·미용실 예약 3건이 새로 틀렸다 — 순손실 −3.8%p. 상세: 카테고리_정확도_측정보고 §8-2.
    "few_shot": [
...
def _category_prompt_section(candidate_names: list[str] | str) -> str:
    ...
    if pol["new_name_examples"]:
        ...
        # ⚠️ 포괄어 금지("업무·정보·생활 금지, 한 단어로") 문구를 넣지 말 것(8/04 실측 기각):
        # 2.5-flash 는 금지된 '업무'(7장) 대신 더 짧고 더 넓은 'IT'(10장)로 도망갔고,
        # 멀쩡히 맞히던 쇼핑 2장·학습 1장까지 삼켰다. 작은 모델에 이름 규칙을 조이면
        # 두더지잡기가 된다. 상세: 카테고리_정확도_측정보고 §8-2.
...
def run_agent_generation(
    ...
    if image_bytes is not None:
        header = (
            ...
            # 판정 축은 "OCR 텍스트의 정보성"이다(오너 원칙, 2026-08-04). 글자의 유무가
            # 아니라 그 글자가 다시 찾아볼 정보인지를 본다 — 소주병 라벨(참이슬)처럼
            # 사물에 인쇄된 상표·문구만 잡힌 실물 사진이 '음식'으로 새던 것을 이 축이 막는다
            # (20장 스모크 실측: 실물사진 4/4 기타, 정보성 스크린샷 역누출 0/8).
            "[정보성] 먼저 판정하라: OCR 로 읽힌 텍스트가 나중에 다시 찾아볼 가치가 있는 "
            "정보(예약·주문·증빙·일정·금액·쿠폰·학습 내용 등)를 담고 있는가. 담고 있지 "
            "않으면 무조건 informative 를 false 로 하고 나머지 필드는 비워 둬라 — 이미지가 "
            "무엇을 찍었든 예외 없다. 카메라로 찍은 실물 사진(음식·물건·풍경)은 상표나 "
            "포장 문구가 읽혀도 그것은 다시 찾아볼 정보가 아니므로 false 다. "
            "순수 UI 요소(홈/검색/설정 내비게이션), 계산기·시계·배터리의 일시적 숫자, "
            "네트워크 오류·로딩·빈 화면도 false 다. "
            "단, 화이트보드·영수증·명함·문서처럼 내용을 기록하려고 글자를 찍은 사진과 "
            "밈·짤·유머 콘텐츠(SNS 후보 대상)는 정보성이다. "
            "정보성일 때만 아래를 채운다.\n\n"
