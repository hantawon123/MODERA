...
원본 이미지의 presigned GET URL(1시간 유효)을 공통 envelope JSON으로 발급한다.
앱이 들고 있던 URL이 만료됐을 때(스토리지 403) 이 API로 새 URL을 받아 재시도하는 용도다.
...
- `url`의 유효시간은 1시간(조회용 GET presign 기존 설정과 동일)
- 같은 `imageId`라도 호출할 때마다 `url` 값은 달라진다(매 요청 신규 서명)
- **만료되는 값이므로 Room 등에 영구 저장하지 않는다.** 이미지 로드가 403이면 이 API를
  다시 불러 새 URL로 재시도한다
...
요청·응답·에러 규칙은 5-8과 같고(성공 code만 `I214`), 대상만 썸네일이다.
