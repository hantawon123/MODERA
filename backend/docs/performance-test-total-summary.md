...
5명·20장 구간에서는 외부 AI `api.example.com`에 대한 90초 read timeout과 DNS 경고가 확인됐다. HTTP 요청 성공만으로 분석 품질까지 성공했다고 판단할 수 없으므로 `COMPLETED`, `FAILED`, `TIMED_OUT`, `EMPTY` 상태를 분리 집계해야 한다.
