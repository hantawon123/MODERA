...
- **6-2 상세의 `ocr.refinedText` 가 "추출된 텍스트"용 교정본입니다.** 융합 분석이
  이미지와 OCR 을 대조해 오인식 줄만 고친 결과이고(줄 삭제·요약은 금지 — 원문과
  줄이 대응돼야 근거로 쓸 수 있습니다), 색인에 함께 저장됩니다. 교정이 없었던
  이미지(비정보성·분석 전·이 기능 이전에 분석된 것)는 `null` 이라 앱이 `rawText` 로
  폴백하면 됩니다. 검색은 계속 `rawText` 로만 합니다.
