...
| 변수 | 로컬 값 | 이유 |
| --- | --- | --- |
| `INTERNAL_TOKEN` | `REDACTED_INTERNAL_TOKEN` | worker 의 `internal.callback.token` 기본값과 **같아야** 한다. 양방향 공유 토큰이라 요청·콜백 양쪽에 쓰인다 |
...
curl -X POST http://localhost:8000/internal/v1/analyze \
  -H 'X-Internal-Token: REDACTED_INTERNAL_TOKEN' \
